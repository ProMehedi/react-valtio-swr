...
const Book = ({ book }: { book: Book }) => {
  ...
  const { id, name, author, thumbnail, featured, price, rating } = book || {}
  ...
  return (
    <div
      className='book-card'
      style={{
        opacity: book?.id === Number(currentId) ? 0.5 : 1,
      }}
    >
      <img
      ...
      <div className='flex-1 h-full pr-2 pt-2 flex flex-col'>
        <div className='flex items-center justify-between'>
          ...
          <div className='text-gray-500 space-x-2'>
            ...
            <button
              ...
              disabled={book?.id === Number(currentId)}
            >
              <svg
                fill='none'
                viewBox='0 0 24 24'
        ...
        <div className='space-y-2 mt-4 h-full'>
          ...
          <p className='lws-price'>{fCurrency(price)}</p>
        </div>
      </div>
    </div>
  )
